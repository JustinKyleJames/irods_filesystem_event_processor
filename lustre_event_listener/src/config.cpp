...
int read_config_file(const std::string& filename, lustre_event_listener_cfg_t *config_struct) {
    ...
    try {
        ...
        if (0 != read_key_from_map(config_map, "mdtname", config_struct->mdtname)) {
            LOG(LOG_ERR, "Key mdtname missing from %s", filename.c_str());
            return irods_filesystem_event_processor_error::CONFIGURATION_ERROR;
        }

        if (0 != read_key_from_map(config_map, "changelog_reader", config_struct->changelog_reader)) {
            LOG(LOG_ERR, "Key changelog_reader missing from %s", filename.c_str());
            return irods_filesystem_event_processor_error::CONFIGURATION_ERROR;
        }

        if (0 != read_key_from_map(config_map, "lustre_root_path", config_struct->lustre_root_path)) {
            LOG(LOG_ERR, "Key lustre_root_path missing from %s", filename.c_str());
            return irods_filesystem_event_processor_error::CONFIGURATION_ERROR;
        }
